    ...
    if ('geolocation' in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(({ coords }) => {
        ...
        fetch(geocodeUrl)
          ...
          .catch((error) => {
            console.error(
              'Error converting latitude and longitude to zipcode:',
              error
            );
          });
      });
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <div className='mb-6 flex w-full gap-4'>
          ...
          <Button
            ...
            className='justify-center rounded-md bg-green-400 p-2 text-white hover:bg-green-600'
            type='submit'
          >
      ...
      {apiRes && (
        <div>
          <p>
            {f('postalCode')}: {apiRes.postalCode}
          </p>
          <p>
            {f('pref')}: {apiRes.city.pref.name}, {apiRes.city.pref.kana}
          </p>
          <p>
            {f('city-district')}: {apiRes.city.name},{apiRes.city.kana}{' '}
          </p>
          <p>
            {f('town')}: {apiRes.name}, {apiRes.kana}
          </p>
        </div>
      )}
      <div>
